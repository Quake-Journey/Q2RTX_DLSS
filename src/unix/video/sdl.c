...
static void mode_changed(void)
{
    ...
    if (flags & SDL_WINDOW_FULLSCREEN) {
        ...
        if (display_index >= 0 &&
            ...
            mode.w > 0 && mode.h > 0) {
            /* On Windows with PerMonitorV2 + SDL_WINDOW_ALLOW_HIGHDPI, Vulkan
             * drawable size can report a DPI-scaled desktop-space extent
             * (for example 5160x2160 on a 3440x1440 monitor at 150% scaling).
             * For fullscreen swapchain sizing we want the actual display mode
             * in physical pixels, not the DPI-scaled drawable size. */
            sdl.width = mode.w;
            sdl.height = mode.h;
