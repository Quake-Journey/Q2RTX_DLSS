...
static void Menu_PushItemValue(void *item)
{
...
void Menu_AddItem(menuFrameWork_t *menu, void *item)
{
	...
	((menuCommon_t *)item)->num_conditions = menu->num_current_conditions;
	if (menu->num_current_conditions > 0) {
		memcpy(((menuCommon_t *)item)->conditions, menu->current_conditions,
			sizeof(menuCondition_t) * menu->num_current_conditions);
	}
}
...
void Menu_Init(menuFrameWork_t *menu)
{
	...
	for (i = 0; i < menu->nitems; i++) {
		item = menu->items[i];
		menuCommon_t *common = (menuCommon_t *)item;
		bool was_hidden = !!(common->flags & QMF_HIDDEN);
		bool hidden = false;

		for (int c = 0; c < common->num_conditions; c++) {
			menuCondition_t *condition = &common->conditions[c];
		...
		if (hidden)
			common->flags |= QMF_HIDDEN;
		else {
			common->flags &= ~QMF_HIDDEN;
			if (was_hidden)
				Menu_PushItemValue(item);
		}
	}
