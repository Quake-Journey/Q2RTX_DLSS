...
static void Parse_If(menuFrameWork_t *menu, bool equals)
{
	...
	if (menu->num_current_conditions >= MAX_MENU_CONDITIONS) {
		Com_Printf("Too many nested ifeq/ifneq blocks\n");
		return;
	}

	menuCondition_t *condition = &menu->current_conditions[menu->num_current_conditions++];
	condition->cvar = Cvar_WeakGet(Cmd_Argv(1));
	condition->value = Q_atoi(Cmd_Argv(2));
