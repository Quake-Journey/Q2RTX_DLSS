...
extern int vkpt_dlss_get_mfg_render_cap(void);
...
extern int  vkpt_dlss_get_display_fps(void);
extern int  vkpt_dlss_get_display_multiplier(void);
extern uint64_t vkpt_dlss_get_total_presented_frames(void);
extern bool vkpt_dlss_mfg_is_enabled(void);
extern bool R_GetDlssDebugOverlay(char *buffer, size_t size);
...
int CL_GetDisplayFps(void)
{
    int fps = vkpt_dlss_get_display_fps();
    return fps > 0 ? fps : R_FPS;
}

int CL_GetDisplayMultiplier(void)
{
    return vkpt_dlss_get_display_multiplier();
}

uint64_t CL_GetDisplayFrameCount(void)
{
    return vkpt_dlss_get_total_presented_frames();
}

bool CL_IsMfgEnabled(void)
{
    return cls.ref_type == REF_TYPE_VKPT && vkpt_dlss_mfg_is_enabled();
}
...
bool CL_GetDlssDebugOverlay(char *buffer, size_t size)
{
	if (cls.ref_type != REF_TYPE_VKPT)
		return false;

	return R_GetDlssDebugOverlay(buffer, size);
}
...
{
    int mfg_render_cap_msec = 0;
    int mfg_render_cap_fps = vkpt_dlss_get_mfg_render_cap();
    if (mfg_render_cap_fps > 0) {
        if (mfg_render_cap_fps < MIN_REF_HZ)
            mfg_render_cap_fps = MIN_REF_HZ;
        else if (mfg_render_cap_fps > MAX_REF_HZ)
            mfg_render_cap_fps = MAX_REF_HZ;
        mfg_render_cap_msec = fps_to_msec(mfg_render_cap_fps);
    }
    ...
    if (com_timedemo->integer) {
        ...
        if (mfg_render_cap_msec > ref_msec)
            ref_msec = mfg_render_cap_msec;
            sync_mode = ASYNC_FULL;
    } else {
        // everything ticks in sync with refresh
        main_msec = fps_to_clamped_msec(cl_maxfps, MIN_PHYS_HZ, MAX_PHYS_HZ);
        if (mfg_render_cap_msec > main_msec)
            main_msec = mfg_render_cap_msec;
