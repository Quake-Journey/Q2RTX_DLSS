...
bool scr_timerefresh_active = false;
...
extern int CL_GetDisplayFps(void);
extern int CL_GetDisplayMultiplier(void);
extern uint64_t CL_GetDisplayFrameCount(void);
extern bool CL_IsMfgEnabled(void);
...
static void SCR_TimeRefresh_f(void)
{
    ...
    float       time;
    float       render_fps;
    uint64_t    display_frames_start = 0;
    uint64_t    display_frames_end = 0;
    bool        vkpt_mode = (cls.ref_type == REF_TYPE_VKPT);
    bool        mfg_enabled = CL_IsMfgEnabled();
    ...
    scr_timerefresh_active = true;
    if (mfg_enabled)
        display_frames_start = CL_GetDisplayFrameCount();
    ...
    if (vkpt_mode) {
        ...
        /* Legacy Quake path: run without page flipping. */
        R_BeginFrame();
        for (i = 0; i < 128; i++) {
    ...
    scr_timerefresh_active = false;
    if (mfg_enabled)
        display_frames_end = CL_GetDisplayFrameCount();

    stop = Sys_Milliseconds();
    time = (stop - start) * 0.001f;
    render_fps = 128.0f / time;

    if (mfg_enabled) {
        uint64_t display_frames = (display_frames_end > display_frames_start)
            ? (display_frames_end - display_frames_start)
            : 0;
        float display_fps = (display_frames > 0)
            ? ((float)display_frames / time)
            : (float)CL_GetDisplayFps();
        Com_Printf("%f seconds (%f render fps, %f display fps)\n", time, render_fps, display_fps);
    } else {
