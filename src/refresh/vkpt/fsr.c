...
static void vkpt_fsr_easu(VkCommandBuffer cmd_buf)
{
	...
	vkpt_dlss_vkCmdBindPipeline(cmd_buf, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_fsr[cvar_flt_fsr_rcas->integer != 0 ? FSR_EASU_TO_RCAS : FSR_EASU_TO_DISPLAY]);
...
static void vkpt_fsr_rcas(VkCommandBuffer cmd_buf)
{
	...
	VkPipeline* pipeline_fsr = qvk.surf_is_hdr ? pipeline_fsr_hdr : pipeline_fsr_sdr;
	vkpt_dlss_vkCmdBindPipeline(cmd_buf, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_fsr[cvar_flt_fsr_easu->integer != 0 ? FSR_RCAS_AFTER_EASU : FSR_RCAS_AFTER_TAAU]);
