...
vkpt_textures_end_registration()
{
	...
	for (int i = 0; i < MAX_RIMAGES; i++)
	{
		...
		if (normalize)
		{
			vkpt_dlss_vkCmdBindPipeline(cmd_buf, VK_PIPELINE_BIND_POINT_COMPUTE, normalize_pipeline);

			vkpt_dlss_vkCmdBindDescriptorSets(cmd_buf, VK_PIPELINE_BIND_POINT_COMPUTE, normalize_pipeline_layout,
...
static VkResult lazy_image_create(vkpt_lazy_image_t *lazy_image, int w, int h, VkFormat format, const char *descr)
{
	...
	VkDeviceSize image_size = 0;
	VkResult create_result = create_image(&image_create_info, &lazy_image->image, &lazy_image->image_mem,
#ifdef VKPT_DEVICE_GROUPS
		lazy_image->image_local,
#else
		NULL,
#endif
		&image_size);
