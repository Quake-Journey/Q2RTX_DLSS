...
vkpt_draw_submit_stretch_pics(VkCommandBuffer cmd_buf)
{
	...
	if (qvk.current_swap_chain_image_index >= (uint32_t)qvk.num_swap_chain_images) {
		Com_EPrintf("[DRAW] stretch_pics: swap image index OOB %u >= %u, skipping\n",
		            qvk.current_swap_chain_image_index, qvk.num_swap_chain_images);
		num_stretch_pics = 0;
		return VK_SUCCESS;
	}
	...
	vkpt_dlss_vkCmdBindDescriptorSets(cmd_buf, VK_PIPELINE_BIND_POINT_GRAPHICS,
			pipeline_layout_stretch_pic, 0, LENGTH(desc_sets), desc_sets, 0, 0);
	vkpt_dlss_vkCmdBindPipeline(cmd_buf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_stretch_pic[qvk.surf_is_hdr ? STRETCH_PIC_HDR : STRETCH_PIC_SDR]);
...
VkResult
vkpt_final_blit_view(VkCommandBuffer cmd_buf, VkImageView src_view, VkExtent2D extent, bool filtered, bool warped)
{
	if (qvk.current_swap_chain_image_index >= (uint32_t)qvk.num_swap_chain_images) {
		Com_EPrintf("[DRAW] final_blit_view: swap image index OOB %u >= %u, skipping\n",
		            qvk.current_swap_chain_image_index, qvk.num_swap_chain_images);
		return VK_SUCCESS;
	}
	...
	vkpt_dlss_vkCmdBindPipeline(cmd_buf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_final_blit[pipeline_idx]);
...
{
	if (qvk.current_swap_chain_image_index >= (uint32_t)qvk.num_swap_chain_images) {
		Com_EPrintf("[DRAW] final_blit: swap image index OOB %u >= %u, skipping\n",
		            qvk.current_swap_chain_image_index, qvk.num_swap_chain_images);
		return VK_SUCCESS;
	}
