...
static cvar_t *cvar_mfg_debug_timing = NULL;
static cvar_t *cvar_flt_dlss_rr_specular_stabilizers = NULL;
static cvar_t *cvar_r_dlss_mip_bias_enable = NULL;
static cvar_t *cvar_r_dlss_mip_bias_global = NULL;
static cvar_t *cvar_r_dlss_mip_bias_min = NULL;
static cvar_t *cvar_r_dlss_mip_bias_max = NULL;
static cvar_t *cvar_r_dlss_mip_bias_ultra_performance_scale = NULL;
static cvar_t *cvar_r_dlss_mip_bias_performance_scale = NULL;
static cvar_t *cvar_r_dlss_mip_bias_balanced_scale = NULL;
static cvar_t *cvar_r_dlss_mip_bias_quality_scale = NULL;
static cvar_t *cvar_r_dlss_mip_bias_ultra_quality_scale = NULL;
static cvar_t *cvar_r_dlss_mip_bias_dlaa_scale = NULL;
static cvar_t *cvar_r_debug_dlss_overlay = NULL;
static cvar_t *cvar_r_debug_accumulation_state = NULL;
static cvar_t *cvar_r_debug_pre_dlss_color = NULL;
static cvar_t *cvar_r_debug_post_dlss_color = NULL;
static cvar_t *cvar_r_debug_post_tonemap_color = NULL;
static cvar_t *cvar_r_debug_dlss_bypass_taa_history = NULL;
static cvar_t *cvar_r_dlss_taa_input_tuning_enable = NULL;
static cvar_t *cvar_r_dlss_taa_input_profile = NULL;
static cvar_t *cvar_r_dlss_taa_history_weight_ultra_performance = NULL;
static cvar_t *cvar_r_dlss_taa_history_weight_performance = NULL;
static cvar_t *cvar_r_dlss_taa_history_weight_balanced = NULL;
...
static bool s_last_reference_enable_accumulation = false;
static bool s_last_reference_enable_denoiser = true;
static float s_last_base_texture_lod_bias = 0.0f;
static float s_last_auto_texture_lod_bias = 0.0f;
static float s_last_effective_texture_lod_bias = 0.0f;
static float s_last_dlss_resolution_scale = 1.0f;
static float s_last_dlss_mip_bias_mode_scale = 0.0f;
static float s_last_effective_taa_history_weight = 0.95f;
static float s_last_effective_taa_anti_sparkle = 0.25f;
static float s_last_effective_taa_variance = 1.0f;
...
typedef struct mfg_debug_timing_stats_s {
	uint64_t sample_count;
	uint64_t reflex_report_count;
	uint64_t last_reflex_report_frame_id;
	double fence_wait_ms_sum;
	double reflex_sleep_ms_sum;
	double present_ms_sum;
	double host_frame_ms_sum;
	double fence_wait_ms_max;
	double reflex_sleep_ms_max;
	double present_ms_max;
	double host_frame_ms_max;
	double reflex_sim_ms_sum;
	double reflex_submit_ms_sum;
	double reflex_present_ms_sum;
	double reflex_driver_ms_sum;
	double reflex_os_queue_ms_sum;
	double reflex_gpu_active_ms_sum;
	double reflex_gpu_frame_ms_sum;
	double reflex_sim_ms_max;
	double reflex_submit_ms_max;
	double reflex_present_ms_max;
	double reflex_driver_ms_max;
	double reflex_os_queue_ms_max;
	double reflex_gpu_active_ms_max;
	double reflex_gpu_frame_ms_max;
} mfg_debug_timing_stats_t;

static mfg_debug_timing_stats_t s_mfg_debug_timing_stats = { 0 };
static uint64_t s_mfg_debug_frame_start_counter = 0;
static double s_mfg_debug_last_fence_wait_ms = 0.0;
static double s_mfg_debug_last_reflex_sleep_ms = 0.0;

static uint64_t mfg_debug_now_counter(void)
{
	return SDL_GetPerformanceCounter();
}

static double mfg_debug_counter_to_ms(uint64_t begin, uint64_t end)
{
	uint64_t freq = SDL_GetPerformanceFrequency();
	if (!freq || end <= begin)
		return 0.0;
	return (double)(end - begin) * 1000.0 / (double)freq;
}

static const char *mfg_debug_present_mode_name(VkPresentModeKHR mode)
{
	switch (mode) {
...
static const char *dlss_mode_name(DlssMode_t mode)
{
	switch (mode) {
	case DLSS_MODE_ULTRA_PERFORMANCE: return "ultra_performance";
	case DLSS_MODE_PERFORMANCE: return "performance";
	case DLSS_MODE_BALANCED: return "balanced";
	case DLSS_MODE_QUALITY: return "quality";
	case DLSS_MODE_ULTRA_QUALITY: return "ultra_quality";
	case DLSS_MODE_DLAA: return "dlaa";
	case DLSS_MODE_CUSTOM: return "custom";
	case DLSS_MODE_OFF:
	default:
		return "off";
	}
...
static const char *mfg_mode_name(DlssMfgMode_t mode)
{
	switch (mode) {
...
static const char *dlss_preset_name(DlssPreset_t preset)
{
	switch (preset) {
	case DLSS_PRESET_D: return "D";
	case DLSS_PRESET_E: return "E";
	case DLSS_PRESET_F: return "F";
	case DLSS_PRESET_J: return "J";
	case DLSS_PRESET_K: return "K";
	case DLSS_PRESET_L: return "L";
...
static const char *dlss_reflex_mode_name(int mode)
{
	switch (mode) {
	case DLSS_REFLEX_ON:       return "on";
	case DLSS_REFLEX_ON_BOOST: return "on+boost";
	case DLSS_REFLEX_OFF:
	default:                   return "off";
	}
}

static void append_overlay_token(char *dst, size_t dst_size, const char *token)
{
	if (!dst || !dst_size || !token || !token[0])
		return;
...
{
	if (!render_extent.width || !render_extent.height || !output_extent.width || !output_extent.height)
		return 1.0f;

	double render_pixels = (double)render_extent.width * (double)render_extent.height;
	double output_pixels = (double)output_extent.width * (double)output_extent.height;
	if (render_pixels <= 0.0 || output_pixels <= 0.0)
		return 1.0f;

	return Q_clipf((float)sqrt(render_pixels / output_pixels), 0.1f, 1.0f);
}

static float get_dlss_mip_bias_mode_scale(DlssMode_t mode)
{
	switch (mode) {
	case DLSS_MODE_ULTRA_PERFORMANCE:
		return cvar_r_dlss_mip_bias_ultra_performance_scale ? cvar_r_dlss_mip_bias_ultra_performance_scale->value : 1.0f;
	case DLSS_MODE_PERFORMANCE:
		return cvar_r_dlss_mip_bias_performance_scale ? cvar_r_dlss_mip_bias_performance_scale->value : 0.85f;
	case DLSS_MODE_BALANCED:
		return cvar_r_dlss_mip_bias_balanced_scale ? cvar_r_dlss_mip_bias_balanced_scale->value : 0.65f;
	case DLSS_MODE_QUALITY:
		return cvar_r_dlss_mip_bias_quality_scale ? cvar_r_dlss_mip_bias_quality_scale->value : 0.45f;
	case DLSS_MODE_ULTRA_QUALITY:
		return cvar_r_dlss_mip_bias_ultra_quality_scale ? cvar_r_dlss_mip_bias_ultra_quality_scale->value : 0.25f;
	case DLSS_MODE_CUSTOM:
		return cvar_r_dlss_mip_bias_quality_scale ? cvar_r_dlss_mip_bias_quality_scale->value : 0.45f;
	case DLSS_MODE_DLAA:
		return cvar_r_dlss_mip_bias_dlaa_scale ? cvar_r_dlss_mip_bias_dlaa_scale->value : 0.0f;
	case DLSS_MODE_OFF:
	default:
		return 0.0f;
	}
...
static float get_dlss_taa_history_weight(DlssMode_t mode, float fallback)
{
	switch (mode) {
	case DLSS_MODE_ULTRA_PERFORMANCE:
		return cvar_r_dlss_taa_history_weight_ultra_performance ? cvar_r_dlss_taa_history_weight_ultra_performance->value : 0.85f;
	case DLSS_MODE_PERFORMANCE:
		return cvar_r_dlss_taa_history_weight_performance ? cvar_r_dlss_taa_history_weight_performance->value : 0.88f;
	case DLSS_MODE_BALANCED:
		return cvar_r_dlss_taa_history_weight_balanced ? cvar_r_dlss_taa_history_weight_balanced->value : 0.91f;
	case DLSS_MODE_QUALITY:
	case DLSS_MODE_ULTRA_QUALITY:
	case DLSS_MODE_CUSTOM:
	case DLSS_MODE_DLAA:
...
static int get_dlss_taa_input_profile(void)
{
	int profile = cvar_r_dlss_taa_input_profile ? cvar_r_dlss_taa_input_profile->integer : 0;
	if (profile < 0)
		return 0;
	if (profile > 2)
		return 2;
...
static bool get_dlss_taa_profile_values(DlssMode_t mode, int profile,
	float *history_weight, float *anti_sparkle_scale, float *variance_scale)
{
	if (profile <= 0)
		return false;
	...
	case 2:
		...
		case DLSS_MODE_ULTRA_PERFORMANCE:
			*history_weight = 0.0f;
			*anti_sparkle_scale = 0.0f;
			*variance_scale = 0.0f;
			return true;
		case DLSS_MODE_PERFORMANCE:
			*history_weight = 0.15f;
			*anti_sparkle_scale = 0.0f;
			*variance_scale = 0.25f;
			return true;
		case DLSS_MODE_BALANCED:
			*history_weight = 0.35f;
			*anti_sparkle_scale = 0.10f;
			*variance_scale = 0.55f;
			return true;
		default:
			return false;
		}
	...
	default:
		switch (mode) {
		case DLSS_MODE_ULTRA_PERFORMANCE:
			*history_weight = 0.10f;
			*anti_sparkle_scale = 0.0f;
			*variance_scale = 0.15f;
			return true;
		case DLSS_MODE_PERFORMANCE:
			*history_weight = 0.25f;
			*anti_sparkle_scale = 0.05f;
			*variance_scale = 0.35f;
			return true;
		case DLSS_MODE_BALANCED:
			*history_weight = 0.50f;
			*anti_sparkle_scale = 0.15f;
			*variance_scale = 0.65f;
			return true;
		default:
			return false;
		}
...
static void apply_dlss_sr_taa_input_tuning(QVKUniformBuffer_t *ubo)
{
	s_last_effective_taa_history_weight = ubo->flt_taa_history_weight;
	s_last_effective_taa_anti_sparkle = ubo->flt_taa_anti_sparkle;
	s_last_effective_taa_variance = ubo->flt_taa_variance;

	if (!cvar_r_dlss_taa_input_tuning_enable || !cvar_r_dlss_taa_input_tuning_enable->integer)
	{
		if (cvar_r_debug_dlss_bypass_taa_history && cvar_r_debug_dlss_bypass_taa_history->integer &&
			vkpt_dlss_is_enabled() && vkpt_dlss_needs_upscale() && !vkpt_dlss_rr_is_enabled())
		{
			ubo->flt_taa_history_weight = 0.0f;
			ubo->flt_taa_anti_sparkle = 0.0f;
			ubo->flt_taa_variance = 0.0f;
			s_last_effective_taa_history_weight = 0.0f;
			s_last_effective_taa_anti_sparkle = 0.0f;
			s_last_effective_taa_variance = 0.0f;
		}
		return;
	}

	if (!vkpt_dlss_is_enabled() || !vkpt_dlss_needs_upscale() || vkpt_dlss_rr_is_enabled())
		return;

	DlssMode_t mode = vkpt_dlss_get_effective_streamline_mode();
	float anti_sparkle_scale = cvar_r_dlss_taa_anti_sparkle_scale ? cvar_r_dlss_taa_anti_sparkle_scale->value : 0.5f;
	float variance_scale = cvar_r_dlss_taa_variance_scale ? cvar_r_dlss_taa_variance_scale->value : 0.85f;
	int profile = get_dlss_taa_input_profile();
	float profile_history_weight = ubo->flt_taa_history_weight;
	float profile_anti_sparkle_scale = anti_sparkle_scale;
	float profile_variance_scale = variance_scale;

	if (get_dlss_taa_profile_values(mode, profile, &profile_history_weight,
		&profile_anti_sparkle_scale, &profile_variance_scale))
	{
		ubo->flt_taa_history_weight = Q_clipf(profile_history_weight, 0.0f, 0.999f);
		ubo->flt_taa_anti_sparkle = max(0.0f, ubo->flt_taa_anti_sparkle * profile_anti_sparkle_scale);
		ubo->flt_taa_variance = max(0.0f, ubo->flt_taa_variance * profile_variance_scale);
	}
	else
	{
		ubo->flt_taa_history_weight = Q_clipf(get_dlss_taa_history_weight(mode, ubo->flt_taa_history_weight), 0.0f, 0.999f);
		ubo->flt_taa_anti_sparkle = max(0.0f, ubo->flt_taa_anti_sparkle * anti_sparkle_scale);
		ubo->flt_taa_variance = max(0.0f, ubo->flt_taa_variance * variance_scale);
	}
...
static float get_legacy_upscale_lod_bias(void)
{
	...
	s_last_dlss_resolution_scale = resolution_scale;
	s_last_dlss_mip_bias_mode_scale = 0.0f;

	return cvar_pt_texture_lod_bias->value + log2f(resolution_scale);
}

static bool is_dlss_rr_active_for_frame(void)
{
	return vkpt_dlss_rr_is_enabled() && !qvk.frame_menu_mode;
}

static float get_effective_texture_lod_bias(bool fsr_enabled, bool dlss_rr_enabled)
{
	float bias = cvar_pt_texture_lod_bias->value;

	s_last_base_texture_lod_bias = cvar_pt_texture_lod_bias->value;
	s_last_auto_texture_lod_bias = 0.0f;
	s_last_dlss_resolution_scale = 1.0f;
	s_last_dlss_mip_bias_mode_scale = 0.0f;

	if (vkpt_dlss_is_enabled() && !dlss_rr_enabled && vkpt_dlss_needs_upscale() &&
		cvar_r_dlss_mip_bias_enable && cvar_r_dlss_mip_bias_enable->integer)
	{
		float resolution_scale = get_extent_resolution_scale(qvk.extent_render, qvk.extent_unscaled);
		float mode_scale = get_dlss_mip_bias_mode_scale(vkpt_dlss_get_effective_streamline_mode());
		float min_bias = cvar_r_dlss_mip_bias_min ? cvar_r_dlss_mip_bias_min->value : -2.0f;
		float max_bias = cvar_r_dlss_mip_bias_max ? cvar_r_dlss_mip_bias_max->value : 0.0f;
		float global_bias = cvar_r_dlss_mip_bias_global ? cvar_r_dlss_mip_bias_global->value : 0.0f;
		float auto_bias = global_bias + log2f(resolution_scale) * mode_scale;

		s_last_dlss_resolution_scale = resolution_scale;
		s_last_dlss_mip_bias_mode_scale = mode_scale;
		s_last_auto_texture_lod_bias = auto_bias;

		bias += auto_bias;
		return Q_clipf(bias, min_bias, max_bias);
	}

	if (fsr_enabled || dlss_rr_enabled || (qvk.effective_aa_mode == AA_MODE_UPSCALE))
		return get_legacy_upscale_lod_bias();
...
bool R_GetDlssDebugOverlay(char *buffer, size_t size)
{
	...
	if (!cvar_r_debug_dlss_overlay || !cvar_r_debug_dlss_overlay->integer || cls.ref_type != REF_TYPE_VKPT)
		return false;

	if (vkpt_dlss_is_enabled())
	{
		dlss_mode = dlss_mode_name(vkpt_dlss_get_mode());
		sr_preset = dlss_preset_name(vkpt_dlss_get_preset());
		sr_dll_version = vkpt_dlss_get_sr_dll_version();
	}
	...
	if (is_dlss_rr_active_for_frame())
		denoiser_state = "replaced";
	...
	line_dlss[0] = 0;
	if (vkpt_dlss_is_enabled())
		append_overlay_token(line_dlss, sizeof(line_dlss), va("DLSS %s", dlss_mode));
	else
...
static void mfg_debug_timing_reset(void)
{
	memset(&s_mfg_debug_timing_stats, 0, sizeof(s_mfg_debug_timing_stats));
}

static void mfg_debug_track_reflex_report(mfg_debug_timing_stats_t *stats)
{
	DlssReflexDebugReport_t report;
	if (!vkpt_dlss_get_latest_reflex_report(&report))
		return;

	if (!report.latency_report_available || !report.frame_id ||
		report.frame_id == stats->last_reflex_report_frame_id)
	{
		return;
	}

	stats->last_reflex_report_frame_id = report.frame_id;
	stats->reflex_report_count++;
	stats->reflex_sim_ms_sum += report.sim_ms;
	stats->reflex_submit_ms_sum += report.submit_ms;
	stats->reflex_present_ms_sum += report.present_ms;
	stats->reflex_driver_ms_sum += report.driver_ms;
	stats->reflex_os_queue_ms_sum += report.os_queue_ms;
	stats->reflex_gpu_active_ms_sum += report.gpu_active_ms;
	stats->reflex_gpu_frame_ms_sum += report.gpu_frame_ms;

	if (report.sim_ms > stats->reflex_sim_ms_max) stats->reflex_sim_ms_max = report.sim_ms;
	if (report.submit_ms > stats->reflex_submit_ms_max) stats->reflex_submit_ms_max = report.submit_ms;
	if (report.present_ms > stats->reflex_present_ms_max) stats->reflex_present_ms_max = report.present_ms;
	if (report.driver_ms > stats->reflex_driver_ms_max) stats->reflex_driver_ms_max = report.driver_ms;
	if (report.os_queue_ms > stats->reflex_os_queue_ms_max) stats->reflex_os_queue_ms_max = report.os_queue_ms;
	if (report.gpu_active_ms > stats->reflex_gpu_active_ms_max) stats->reflex_gpu_active_ms_max = report.gpu_active_ms;
	if (report.gpu_frame_ms > stats->reflex_gpu_frame_ms_max) stats->reflex_gpu_frame_ms_max = report.gpu_frame_ms;
}

static void mfg_debug_timing_record(double fence_wait_ms, double reflex_sleep_ms,
	double present_ms, double host_frame_ms)
{
	if (!cvar_mfg_debug_timing || !cvar_mfg_debug_timing->integer || qvk.frame_menu_mode)
	{
		mfg_debug_timing_reset();
		return;
	}

	mfg_debug_timing_stats_t *stats = &s_mfg_debug_timing_stats;
	stats->sample_count++;
	stats->fence_wait_ms_sum += fence_wait_ms;
	stats->reflex_sleep_ms_sum += reflex_sleep_ms;
	stats->present_ms_sum += present_ms;
	stats->host_frame_ms_sum += host_frame_ms;
	if (fence_wait_ms > stats->fence_wait_ms_max) stats->fence_wait_ms_max = fence_wait_ms;
	if (reflex_sleep_ms > stats->reflex_sleep_ms_max) stats->reflex_sleep_ms_max = reflex_sleep_ms;
	if (present_ms > stats->present_ms_max) stats->present_ms_max = present_ms;
	if (host_frame_ms > stats->host_frame_ms_max) stats->host_frame_ms_max = host_frame_ms;
	mfg_debug_track_reflex_report(stats);

	if (stats->sample_count >= 120) {
		double inv = 1.0 / (double)stats->sample_count;
		double report_inv = stats->reflex_report_count ? (1.0 / (double)stats->reflex_report_count) : 0.0;
		int display_multiplier = vkpt_dlss_get_display_multiplier();
		if (display_multiplier <= 0)
			display_multiplier = 1;
		int display_fps = vkpt_dlss_get_display_fps();
		if (display_fps <= 0 && stats->host_frame_ms_sum > 0.0)
			display_fps = (int)llround(1000.0 / (stats->host_frame_ms_sum * inv));
		Com_Printf("[MFGDBG] mode=%d mult=%d display_fps=%d render=%ux%u display=%ux%u "
			"dg_status=0x%x dg_presented=%d dg_vsync=%d reflex_eff=%d surf_vsync=%d present_mode=%s swap_images=%d "
			"avg_ms(fence=%.3f reflex=%.3f present=%.3f host=%.3f) "
			"rpt(count=%llu last=%llu avg(sim=%.3f submit=%.3f present=%.3f driver=%.3f osq=%.3f gpu=%.3f gpu_frame=%.3f) "
			"max(sim=%.3f submit=%.3f present=%.3f driver=%.3f osq=%.3f gpu=%.3f gpu_frame=%.3f)) "
			"max_ms(fence=%.3f reflex=%.3f present=%.3f host=%.3f)\n",
			(int)vkpt_dlss_get_mfg_mode(),
			display_multiplier,
			display_fps,
			qvk.extent_render.width, qvk.extent_render.height,
			qvk.extent_unscaled.width, qvk.extent_unscaled.height,
			vkpt_dlss_get_last_dlssg_status(),
			vkpt_dlss_get_last_dlssg_frames_presented(),
			vkpt_dlss_get_last_dlssg_vsync_support(),
			vkpt_dlss_get_effective_reflex_mode(),
			qvk.surf_vsync ? 1 : 0,
			mfg_debug_present_mode_name(qvk.present_mode),
			qvk.num_swap_chain_images,
			stats->fence_wait_ms_sum * inv,
			stats->reflex_sleep_ms_sum * inv,
			stats->present_ms_sum * inv,
			stats->host_frame_ms_sum * inv,
			(unsigned long long)stats->reflex_report_count,
			(unsigned long long)stats->last_reflex_report_frame_id,
			stats->reflex_sim_ms_sum * report_inv,
			stats->reflex_submit_ms_sum * report_inv,
			stats->reflex_present_ms_sum * report_inv,
			stats->reflex_driver_ms_sum * report_inv,
			stats->reflex_os_queue_ms_sum * report_inv,
			stats->reflex_gpu_active_ms_sum * report_inv,
			stats->reflex_gpu_frame_ms_sum * report_inv,
			stats->reflex_sim_ms_max,
			stats->reflex_submit_ms_max,
			stats->reflex_present_ms_max,
			stats->reflex_driver_ms_max,
			stats->reflex_os_queue_ms_max,
			stats->reflex_gpu_active_ms_max,
			stats->reflex_gpu_frame_ms_max,
			stats->fence_wait_ms_max,
			stats->reflex_sleep_ms_max,
			stats->present_ms_max,
			stats->host_frame_ms_max);
		mfg_debug_timing_reset();
	}
}
...
VkptInit_t vkpt_initialization[] = {
	...
	{ "dlss_rr",  vkpt_dlss_rr_initialize,             vkpt_dlss_rr_destroy,                 VKPT_INIT_DEFAULT,            0 },
	{ "dlss_rr|", vkpt_dlss_rr_create_pipelines,       vkpt_dlss_rr_destroy_pipelines,       VKPT_INIT_RELOAD_SHADER,      0 },
...
static void vkpt_submit_command_buffer_with_timeline_wait(
	...
	VkSemaphore timeline_wait_semaphore,
...
{
	/* DLSS SR: render at optimal sub-resolution determined by the quality mode cvar */
	if (vkpt_dlss_is_enabled() && vkpt_dlss_needs_upscale())
	{
		uint32_t render_w = qvk.extent_unscaled.width;
		uint32_t render_h = qvk.extent_unscaled.height;
		vkpt_dlss_get_render_resolution(
			qvk.extent_unscaled.width, qvk.extent_unscaled.height,
			&render_w, &render_h);
		VkExtent2D result;
		result.width  = (render_w + 1) & ~1;
...
{
	dlss_sl_vkDeviceWaitIdle(qvk.device);
...
static bool pick_surface_format_sdr(picked_surface_format_t* picked_fmt, const VkSurfaceFormatKHR avail_surface_formats[], size_t num_avail_surface_formats, bool prefer_unorm)
{
	typedef struct {
		VkFormat format;
		VkFormat swapchain_view_fmt;
	} swapchain_format_pref_t;
	static const swapchain_format_pref_t acceptable_formats_prefer_unorm[] = {
		{VK_FORMAT_R8G8B8A8_UNORM, VK_FORMAT_R8G8B8A8_SRGB},
		{VK_FORMAT_B8G8R8A8_UNORM, VK_FORMAT_B8G8R8A8_SRGB},
		{VK_FORMAT_R8G8B8A8_SRGB, VK_FORMAT_R8G8B8A8_SRGB},
		{VK_FORMAT_B8G8R8A8_SRGB, VK_FORMAT_B8G8R8A8_SRGB},
	};
	static const swapchain_format_pref_t acceptable_formats_prefer_srgb[] = {
		{VK_FORMAT_R8G8B8A8_SRGB, VK_FORMAT_R8G8B8A8_SRGB},
		{VK_FORMAT_B8G8R8A8_SRGB, VK_FORMAT_B8G8R8A8_SRGB},
		{VK_FORMAT_R8G8B8A8_UNORM, VK_FORMAT_R8G8B8A8_SRGB},
		{VK_FORMAT_B8G8R8A8_UNORM, VK_FORMAT_B8G8R8A8_SRGB},
	};
	const size_t num_formats_to_try = LENGTH(acceptable_formats_prefer_unorm);
	const swapchain_format_pref_t *acceptable_formats =
		prefer_unorm ? acceptable_formats_prefer_unorm : acceptable_formats_prefer_srgb;

	for(int i = 0; i < (int)num_formats_to_try; i++) {
		for(int j = 0; j < num_avail_surface_formats; j++) {
			if(acceptable_formats[i].format == avail_surface_formats[j].format) {
				picked_fmt->surface_fmt = avail_surface_formats[j];
...
create_swapchain(void)
{
	...
	qvk.surf_mfg_enabled = vkpt_dlss_mfg_is_enabled();
	...
	/* Use SL-patched swapchain functions so DLSS-G can intercept swapchain creation.
	 * These fall back to the bare vk* equivalents when SL is not loaded. */
	if(dlss_sl_vkCreateSwapchainKHR(qvk.device, &swpch_create_info, NULL, &qvk.swap_chain) != VK_SUCCESS) {
		Com_EPrintf("error creating swapchain\n");
		return 1;
	}

	dlss_sl_vkGetSwapchainImagesKHR(qvk.device, qvk.swap_chain, &qvk.num_swap_chain_images, NULL);
	assert(qvk.num_swap_chain_images);
	qvk.swap_chain_images = malloc(qvk.num_swap_chain_images * sizeof(*qvk.swap_chain_images));
	dlss_sl_vkGetSwapchainImagesKHR(qvk.device, qvk.swap_chain, &qvk.num_swap_chain_images, qvk.swap_chain_images);
...
static bool
string_list_contains(const char **list, uint32_t count, const char *value)
{
...
static void
append_string_unique(const char** dst, uint32_t* dst_count, uint32_t dst_capacity, const char* value)
{
	if (!value || string_list_contains(dst, *dst_count, value))
		return;
	assert(*dst_count + 1 <= dst_capacity);
	dst[*dst_count] = value;
	(*dst_count)++;
}
...
init_vulkan(void)
{
	...
	int num_inst_ext_max = qvk.num_sdl2_extensions
		+ LENGTH(vk_requested_instance_extensions)
		+ NUM_OPTIONAL_INSTANCE_EXTENSIONS
		+ (int)vkpt_dlss_get_required_instance_extension_count();
	...
	for (uint32_t i = 0; i < vkpt_dlss_get_required_instance_extension_count(); ++i) {
		const char *req_ext = vkpt_dlss_get_required_instance_extension(i);
		if (!req_ext || string_list_contains(ext, (uint32_t)num_inst_ext_combined, req_ext))
			continue;
		assert(num_inst_ext_combined + 1 <= num_inst_ext_max);
		ext[num_inst_ext_combined++] = req_ext;
	}
	 ...
	 * Without this, sl.interposer's vkEnumeratePhysicalDevices/vkCreateDevice hooks
	 * crash on s_idt null-pointer access (CR45/CR46). Falls back to vkCreateInstance. */
	...
	if (result == VK_ERROR_LAYER_NOT_PRESENT)
	{
		...
		result = fn_ci(&inst_create_info, NULL, &qvk.instance);
		qvk.enable_validation = false;
	}
...
#undef VK_EXTENSION_DO
	...
	VkPhysicalDeviceVulkan13Features supported_features_vk13 = {
		.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_3_FEATURES,
	};
	VkPhysicalDeviceOpticalFlowFeaturesNV supported_optical_flow_features = {
	...
	}
	{
		VkPhysicalDeviceFeatures2 supported_features = {
			...
			.pNext = &supported_features_vk13,
		};
		supported_features_vk13.pNext = &supported_optical_flow_features;
		vkGetPhysicalDeviceFeatures2(qvk.physical_device, &supported_features);
	}
	...
	qvk.queue_idx_optical_flow = -1;
	qvk.queue_present = VK_NULL_HANDLE;
	qvk.sl_graphics_queue_index = 1;
	qvk.sl_compute_queue_index = 1;
	qvk.sl_optical_flow_queue_index = 0;
	qvk.sl_use_native_optical_flow = false;
	...
	if (supported_features_vk13.synchronization2 && supported_optical_flow_features.opticalFlow) {
		for (int i = 0; i < num_queue_families; i++) {
			if (!queue_families[i].queueCount)
				continue;
			if (i == qvk.queue_idx_graphics)
				continue;
			if ((queue_families[i].queueFlags & VK_QUEUE_OPTICAL_FLOW_BIT_NV) == VK_QUEUE_OPTICAL_FLOW_BIT_NV) {
				qvk.queue_idx_optical_flow = i;
				qvk.sl_optical_flow_queue_index = 0;
				qvk.sl_use_native_optical_flow = true;
				break;
			}
		}
	}
	...
	float queue_priorities[8] = {1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f};
	int num_create_queues = 0;
	VkDeviceQueueCreateInfo queue_create_info[3];

	{
		uint32_t available_graphics_queues = queue_families[qvk.queue_idx_graphics].queueCount;
		/* Keep render and present on separate host queues when the family exposes
		 * at least two queues. DLSS-G default pacing blocks the presenting client
		 * queue; if render submits and present share queue 0, FG stalls host render
		 * cadence almost proportionally to the multiplier. */
		uint32_t host_graphics_queues = (available_graphics_queues >= 2) ? 2u : 1u;
		uint32_t sl_extra_graphics_queues = vkpt_dlss_get_required_graphics_queue_count();
		uint32_t sl_extra_compute_queues = vkpt_dlss_get_required_compute_queue_count();
		uint32_t total_graphics_family_queues =
			host_graphics_queues + sl_extra_graphics_queues + sl_extra_compute_queues;

		if (total_graphics_family_queues > available_graphics_queues) {
			host_graphics_queues = 1;
			total_graphics_family_queues =
				host_graphics_queues + sl_extra_graphics_queues + sl_extra_compute_queues;
		}

		qvk.sl_graphics_queue_index = host_graphics_queues;
		qvk.sl_compute_queue_index = host_graphics_queues + sl_extra_graphics_queues;

		VkDeviceQueueCreateInfo q = {
			.sType            = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
			.queueCount       = total_graphics_family_queues,
	...
	if (qvk.sl_use_native_optical_flow && qvk.queue_idx_optical_flow >= 0) {
		uint32_t sl_extra_optical_flow_queues = vkpt_dlss_get_required_optical_flow_queue_count();
		if (sl_extra_optical_flow_queues > 0) {
			VkDeviceQueueCreateInfo q = {
				.sType            = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
				.queueCount       = sl_extra_optical_flow_queues,
				.pQueuePriorities = queue_priorities,
				.queueFamilyIndex = (uint32_t)qvk.queue_idx_optical_flow,
			};
			queue_create_info[num_create_queues++] = q;
		}
	}
	...
	VkPhysicalDeviceVulkan12Features device_features_vk12 = {
		...
		.timelineSemaphore = vkpt_dlss_requires_vk_feature12("timelineSemaphore") ? VK_TRUE : VK_FALSE,
	};
	VkPhysicalDeviceVulkan13Features device_features_vk13 = {
		.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_3_FEATURES,
		.synchronization2 = (vkpt_dlss_requires_vk_feature13("synchronization2") && supported_features_vk13.synchronization2) ? VK_TRUE : VK_FALSE,
	};
	VkPhysicalDeviceOpticalFlowFeaturesNV device_features_optical_flow = {
		.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_OPTICAL_FLOW_FEATURES_NV,
		.opticalFlow = qvk.sl_use_native_optical_flow ? VK_TRUE : VK_FALSE,
	};
	VkPhysicalDeviceFeatures2 device_features = {
		.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2_KHR,
	...
	VkBaseOutStructure *device_features_tail = (VkBaseOutStructure*)&device_features_vk12;
	if (device_features_vk13.synchronization2) {
		device_features_tail->pNext = (VkBaseOutStructure*)&device_features_vk13;
		device_features_tail = (VkBaseOutStructure*)&device_features_vk13;
	}
	if (qvk.sl_use_native_optical_flow) {
		device_features_tail->pNext = (VkBaseOutStructure*)&device_features_optical_flow;
		device_features_tail = (VkBaseOutStructure*)&device_features_optical_flow;
	}
	...
	max_extension_count += vkpt_dlss_get_required_device_extension_count();
	...
	if (qvk.use_ray_query)
	{
		...
		device_features_tail->pNext = (VkBaseOutStructure*)&physical_device_ray_query_features;
	}
	else
	{
		append_string_list(device_extensions, &device_extension_count, max_extension_count,
		...
		device_features_tail->pNext = (VkBaseOutStructure*)&physical_device_rt_pipeline_features;
	}
	...
	for (uint32_t i = 0; i < vkpt_dlss_get_required_device_extension_count(); ++i) {
		append_string_unique(device_extensions, &device_extension_count, max_extension_count,
			vkpt_dlss_get_required_device_extension(i));
	}
	...
	if (qvk.sl_graphics_queue_index >= 2)
		vkGetDeviceQueue(qvk.device, qvk.queue_idx_graphics, 1, &qvk.queue_present);
	else
...
destroy_swapchain(void)
{
	...
	dlss_sl_vkDestroySwapchainKHR(qvk.device, qvk.swap_chain, NULL);
...
destroy_vulkan(void)
{
	...
	destroy_swapchain();
	dlss_sl_vkDestroySurfaceKHR(qvk.instance, qvk.surface, NULL);
...
evaluate_reference_mode(reference_mode_t* ref_mode)
{
	...
	s_last_reference_enable_accumulation = ref_mode->enable_accumulation;
	s_last_reference_enable_denoiser = ref_mode->enable_denoiser;
}
...
evaluate_taa_settings(const reference_mode_t* ref_mode)
{
	...
	bool dlss_rr_active = is_dlss_rr_active_for_frame();
	if (!ref_mode->enable_denoiser && !dlss_rr_active)
		return;
	...
	if (dlss_rr_active)
	{
		/* RR replaces TAA/TAAU but still needs the temporal-jittered ray path.
		 * Keep AA_MODE_TAA for camera jitter and history reset logic, but render the
		 * actual reconstruction output directly into display-resolution TAA_OUTPUT. */
		qvk.effective_aa_mode = AA_MODE_TAA;
		qvk.extent_taa_output = qvk.extent_unscaled;
		return;
	}

	/* DLSS SR upscales internally and must receive render-resolution TAA output.
	 * If we force AA_MODE_UPSCALE (TAAU), the TAA shader writes a full display-res
	 * image into IMG_TAA_OUTPUT; DLSS then treats the render-sized sub-rect of that
	 * display-space image as render-space data and upscales it again → zoomed result.
	 *
	 * Fix: when DLSS SR is active, force AA_MODE_TAA so TAA runs at render resolution
	 * and extent_taa_output stays at extent_render.  DLSS receives the correct
	 * render-res content and handles the upscale to display resolution itself.
	 *
	 * FSR still needs the TAAU path (AA_MODE_UPSCALE) so that FSR EASU receives a
	 * temporally-accumulated render-res image ready for spatial upscaling. */
	...
	if (dlss_sr_active)
	{
		flt_taa = AA_MODE_TAA; /* render-res TAA; DLSS upscales to display */
	}
...
#undef UBO_CVAR_DO
	...
	apply_dlss_sr_taa_input_tuning(ubo);

	bool fsr_enabled = vkpt_fsr_is_enabled();
	bool dlss_rr_enabled = vkpt_dlss_rr_is_enabled();
	bool dlss_rr_active = is_dlss_rr_active_for_frame();
	bool shader_temporal_filter_active = ref_mode->enable_denoiser || dlss_rr_active;
	...
	if (!ref_mode->enable_denoiser && !dlss_rr_active)
	{
		...
		/* Legacy Q2RTX TAA path flips checkerboard fields every frame to accumulate
		 * both reflection and refraction branches into the same pixel over time.
		 * DLSS-RR already performs its own temporal reconstruction; feeding it an
		 * alternating split-path pattern creates residual heat-haze shimmer on
		 * reflective floors/walls. Keep the checkerboard assignment stable while
		 * RR is active. */
		ubo->pt_swap_checkerboard = vkpt_dlss_rr_is_enabled() ? 0 : (qvk.frame_counter & 1);

		if (ref_mode->enable_accumulation)
		{
			ubo->pt_texture_lod_bias = -log2f(sqrtf(get_accumulation_rendering_framenum()));
			s_last_effective_texture_lod_bias = ubo->pt_texture_lod_bias;
			s_last_dlss_resolution_scale = 1.0f;
			s_last_dlss_mip_bias_mode_scale = 0.0f;
	...
	else if (vkpt_dlss_is_enabled() || fsr_enabled || dlss_rr_enabled || (qvk.effective_aa_mode == AA_MODE_UPSCALE))
	{
		ubo->pt_texture_lod_bias = get_effective_texture_lod_bias(fsr_enabled, dlss_rr_enabled);
		s_last_effective_texture_lod_bias = ubo->pt_texture_lod_bias;

		if (dlss_rr_active)
		{
			/* These legacy q2rtx noise-reduction hacks are tuned for the ASVGF/TAA
			 * denoiser path. RR needs the real noisy specular signal instead of the
			 * fallback SH/fake-specular path and distance-based anti-flicker fades. */
			const int rr_stabilizers_mode = cvar_flt_dlss_rr_specular_stabilizers->integer;

			if (rr_stabilizers_mode == 0)
			{
				ubo->pt_fake_roughness_threshold = 1.f;
				ubo->pt_ndf_trim = 1.f;
				ubo->pt_specular_anti_flicker = 0.f;
	...
	ubo->flt_enable = shader_temporal_filter_active;
	...
	if (dlss_rr_active)
	{
		/* RR consumes explicit sub-pixel jitter through Streamline constants.
		 * Q2RTX's AA_MODE_TAA path makes primary_rays.rgen use per-pixel random
		 * jitter instead of global_ubo.sub_pixel_jitter, which does not match the
		 * jitterOffset we pass to RR and leads to static/dynamic ghosting.
		 *
		 * Keep the higher-level RR scheduling/extents, but make the raygen path use
		 * the deterministic upscale-style jitter pattern. */
		ubo->flt_taa = AA_MODE_UPSCALE;
	}
	...
	if (qvk.effective_aa_mode == AA_MODE_UPSCALE || dlss_rr_enabled)
	{
...
R_RenderFrame_RTX(refdef_t *fd)
{
	...
	/* Reflex/PCL marker chain:
	 * - SimulationStart is emitted after Reflex sleep, when CPU work for the new frame actually begins
	 * - SimulationEnd marks the end of CPU simulation/build work for this frame
	 * - RenderSubmitStart/End bracket the GPU submit phase
	 *
	 * Without these markers Reflex sleep falls back to a conservative pacing path,
	 * which shows up as an artificial host-FPS cap when DLSS-G is enabled. */
	vkpt_dlss_reflex_mark_simulation_end();
	...
	VkSemaphore frame_start_wait_semaphores[VKPT_MAX_GPUS + 1];
	VkPipelineStageFlags frame_start_wait_stages[VKPT_MAX_GPUS + 1];
	uint32_t frame_start_wait_device_indices[VKPT_MAX_GPUS + 1];
	VkSemaphore fg_inputs_wait_semaphore = VK_NULL_HANDLE;
	uint64_t fg_inputs_wait_value = 0;
	int fg_inputs_wait_active = 0;
...

	{
		...
		int frame_start_wait_count = 0;
		frame_start_wait_semaphores[frame_start_wait_count] = qvk.semaphores[qvk.current_frame_index][0].image_available;
		frame_start_wait_stages[frame_start_wait_count] = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
		frame_start_wait_device_indices[frame_start_wait_count] = 0;
		frame_start_wait_count++;
		...
		for (int gpu = 0; gpu < qvk.device_count; gpu++)
		{
			...
			if (*prev_trace_signaled && frame_start_wait_count < VKPT_MAX_GPUS + 1)
			{
				frame_start_wait_semaphores[frame_start_wait_count] = prev_trace_semaphores[gpu];
				frame_start_wait_stages[frame_start_wait_count] = wait_stages[gpu];
				frame_start_wait_device_indices[frame_start_wait_count] = device_indices[gpu];
				frame_start_wait_count++;
			}
		}

		vkpt_dlss_reflex_mark_render_submit_start();

		vkpt_submit_command_buffer(
			transfer_cmd_buf,
			qvk.queue_transfer,
			all_device_mask,
			frame_start_wait_count, frame_start_wait_semaphores, frame_start_wait_stages, frame_start_wait_device_indices,
...

	{
		...
		fg_inputs_wait_active = vkpt_dlss_get_frame_generation_inputs_wait(
			&fg_inputs_wait_semaphore, &fg_inputs_wait_value);

		vkpt_submit_command_buffer_with_timeline_wait(
			trace_cmd_buf,
			qvk.queue_graphics,
			all_device_mask,
			qvk.device_count, transfer_semaphores, wait_stages, device_indices,
			fg_inputs_wait_active ? fg_inputs_wait_semaphore : VK_NULL_HANDLE,
			fg_inputs_wait_active ? fg_inputs_wait_value : 0,
			0, 0, 0,
			VK_NULL_HANDLE);

		if (fg_inputs_wait_active)
			vkpt_dlss_mark_frame_generation_inputs_waited(fg_inputs_wait_value);
	}
...

	{
		...
		if (ref_mode.enable_denoiser && !(vkpt_dlss_rr_is_enabled() && !qvk.frame_menu_mode))
		{
...

	{
		...
		{
			/* RR replaces the denoiser path. Feed it the noisy composited PT color,
			   not the already denoised ASVGF output. */
			vkpt_compositing(post_cmd_buf);
		}
		else if (ref_mode.enable_denoiser)
		{
		...
		if (vkpt_dlss_rr_is_enabled() && !qvk.frame_menu_mode)
			vkpt_dlss_rr_process(post_cmd_buf);
		else
	...
	temporal_frame_valid = ref_mode.enable_denoiser || (vkpt_dlss_rr_is_enabled() && !qvk.frame_menu_mode);
...
static void
refresh_drawable_size_from_window(void)
{
...
{
	LOG_FUNC();
	s_mfg_debug_frame_start_counter = mfg_debug_now_counter();

	qvk.current_frame_index = qvk.frame_counter % MAX_FRAMES_IN_FLIGHT;

	uint64_t fence_wait_begin = mfg_debug_now_counter();
	VkResult res_fence = vkWaitForFences(qvk.device, 1, qvk.fences_frame_sync + qvk.current_frame_index, VK_TRUE, ~((uint64_t) 0));
	s_mfg_debug_last_fence_wait_ms = mfg_debug_counter_to_ms(fence_wait_begin, mfg_debug_now_counter());
	...
	{
		if (mode_changed) {
			refresh_drawable_size_from_window();
		}
	...
	bool taa_image_extent_changed = false;
	bool mfg_state_changed = (qvk.surf_mfg_enabled != vkpt_dlss_mfg_is_enabled());

	/* get_screen_image_extent() depends on qvk.extent_render, so temporarily
	 * compute it against the candidate render extent before deciding whether a
	 * swapchain/image recreate is required. Without this, switching DLSS modes
	 * (for example Ultra Performance -> Ultra Quality) or enabling RR can leave
	 * render-sized images at the old resolution while Streamline receives the new
	 * render subrect, which freezes the world image and keeps only HUD updates. */
	{
		VkExtent2D old_extent_render = qvk.extent_render;
		qvk.extent_render = extent_render;
		extent_screen_images = get_screen_image_extent();
		qvk.extent_render = old_extent_render;
	}

	extent_taa_images.width = max(extent_screen_images.width, qvk.extent_unscaled.width);
	extent_taa_images.height = max(extent_screen_images.height, qvk.extent_unscaled.height);

	render_extent_changed = !extents_equal(extent_render, qvk.extent_render);
	taa_image_extent_changed = !extents_equal(extent_taa_images, qvk.extent_taa_images);

	if(render_extent_changed ||
		taa_image_extent_changed ||
		!extents_equal(extent_screen_images, qvk.extent_screen_images) ||
		(!!cvar_hdr->integer != qvk.surf_is_hdr) ||
		(!!cvar_vsync->integer != qvk.surf_vsync) ||
		mfg_state_changed)
	{
		qvk.extent_render = extent_render;
		qvk.extent_screen_images = extent_screen_images;
		qvk.extent_taa_images = extent_taa_images;
		qvk.gpu_slice_width = (qvk.extent_render.width + qvk.device_count - 1) / qvk.device_count;
		recreate_swapchain();
	}
	else
...
retry:;
...
#else
		VkResult res_swapchain = dlss_sl_vkAcquireNextImageKHR(qvk.device, qvk.swap_chain, ~((uint64_t) 0),
	...
	vkpt_dlss_begin_frame((uint32_t)(qvk.frame_counter & 0xFFFFFFFF));

	/* Vulkan Reflex SDK recommends calling sleep at the very start of each frame.
	 * Keeping it later in the frame leaves extra housekeeping work in front of the
	 * driver pacer and can distort render cadence under DLSS-G. */
	uint64_t reflex_sleep_begin = mfg_debug_now_counter();
	vkpt_dlss_reflex_sleep();
	s_mfg_debug_last_reflex_sleep_ms =
		mfg_debug_counter_to_ms(reflex_sleep_begin, mfg_debug_now_counter());

	/* Start the CPU simulation/build phase only after Reflex sleep has completed.
	 * If SimulationStart is emitted before sleep, PCL can account the sleep interval
	 * as part of the simulation phase and over-throttle host pacing under DLSS-G. */
	vkpt_dlss_reflex_mark_simulation_start();
...
R_EndFrame_RTX(void)
{
	...
	if (frame_ready)
	{
		...
		bool debug_pre_dlss = cvar_r_debug_pre_dlss_color && cvar_r_debug_pre_dlss_color->integer;
		bool debug_post_dlss = cvar_r_debug_post_dlss_color && cvar_r_debug_post_dlss_color->integer;
		bool debug_post_tonemap = cvar_r_debug_post_tonemap_color && cvar_r_debug_post_tonemap_color->integer;
		VkImageView dlss_output_view = vkpt_dlss_get_output_view();

		if (debug_post_dlss && vkpt_dlss_is_enabled() && !vkpt_dlss_rr_is_enabled() &&
			!qvk.frame_menu_mode && dlss_output_view != VK_NULL_HANDLE)
		{
			vkpt_final_blit_view(cmd_buf, dlss_output_view, qvk.extent_unscaled, false, waterwarp);
		}
		else if ((debug_pre_dlss || debug_post_tonemap) && !qvk.frame_menu_mode)
		{
			vkpt_final_blit(cmd_buf, VKPT_IMG_TAA_OUTPUT, qvk.extent_taa_output, false, waterwarp);
		}
		else if (vkpt_dlss_rr_is_enabled() && !qvk.frame_menu_mode)
		{
			vkpt_final_blit(cmd_buf, VKPT_IMG_TAA_OUTPUT, qvk.extent_taa_output, false, waterwarp);
		}
	...
	vkpt_dlss_reflex_mark_render_submit_end();
	...
	uint64_t present_begin = mfg_debug_now_counter();
	VkResult res_present = dlss_sl_vkQueuePresentKHR(qvk.queue_graphics, &present_info);
...
R_Init_RTX(bool total)
{
    ...
    /* Apply the requested video mode before creating Vulkan objects.
     * Otherwise startup happens in two phases:
     *   1) create swapchain using startup window geometry
     *   2) refresh layer later calls vid.set_mode(), triggering a second
     *      fullscreen transition and swapchain recreate
     *
     * DLSS-G attaches to the early swapchain and the late transition can
     * recreate it at an oversized desktop/DPI-scaled extent, causing
     * unnecessary FG cost.  Applying the mode here makes the first
     * swapchain the final one. */
    vid.set_mode();
	...
	cvar_mfg_debug_timing = Cvar_Get("flt_dlss_mfg_debug_timing", "0", CVAR_ARCHIVE);
	cvar_r_dlss_mip_bias_enable = Cvar_Get("r_dlss_mip_bias_enable", "1", CVAR_ARCHIVE);
	cvar_r_dlss_mip_bias_global = Cvar_Get("r_dlss_mip_bias_global", "0.0", CVAR_ARCHIVE);
	cvar_r_dlss_mip_bias_min = Cvar_Get("r_dlss_mip_bias_min", "-2.0", CVAR_ARCHIVE);
	cvar_r_dlss_mip_bias_max = Cvar_Get("r_dlss_mip_bias_max", "0.0", CVAR_ARCHIVE);
	cvar_r_dlss_mip_bias_ultra_performance_scale = Cvar_Get("r_dlss_mip_bias_ultra_performance_scale", "1.0", CVAR_ARCHIVE);
	cvar_r_dlss_mip_bias_performance_scale = Cvar_Get("r_dlss_mip_bias_performance_scale", "0.85", CVAR_ARCHIVE);
	cvar_r_dlss_mip_bias_balanced_scale = Cvar_Get("r_dlss_mip_bias_balanced_scale", "0.65", CVAR_ARCHIVE);
	cvar_r_dlss_mip_bias_quality_scale = Cvar_Get("r_dlss_mip_bias_quality_scale", "0.45", CVAR_ARCHIVE);
	cvar_r_dlss_mip_bias_ultra_quality_scale = Cvar_Get("r_dlss_mip_bias_ultra_quality_scale", "0.25", CVAR_ARCHIVE);
	cvar_r_dlss_mip_bias_dlaa_scale = Cvar_Get("r_dlss_mip_bias_dlaa_scale", "0.0", CVAR_ARCHIVE);
	cvar_flt_dlss_rr_specular_stabilizers = Cvar_Get("flt_dlss_rr_specular_stabilizers", "2", CVAR_ARCHIVE);
	cvar_r_debug_dlss_overlay = Cvar_Get("r_debug_dlss_overlay", "0", CVAR_ARCHIVE);
	cvar_r_debug_accumulation_state = Cvar_Get("r_debug_accumulation_state", "0", CVAR_ARCHIVE);
	cvar_r_debug_pre_dlss_color = Cvar_Get("r_debug_pre_dlss_color", "0", CVAR_ARCHIVE);
	cvar_r_debug_post_dlss_color = Cvar_Get("r_debug_post_dlss_color", "0", CVAR_ARCHIVE);
	cvar_r_debug_post_tonemap_color = Cvar_Get("r_debug_post_tonemap_color", "0", CVAR_ARCHIVE);
	cvar_r_debug_dlss_bypass_taa_history = Cvar_Get("r_debug_dlss_bypass_taa_history", "0", CVAR_ARCHIVE);
	cvar_r_dlss_taa_input_tuning_enable = Cvar_Get("r_dlss_taa_input_tuning_enable", "1", CVAR_ARCHIVE);
	cvar_r_dlss_taa_input_profile = Cvar_Get("r_dlss_taa_input_profile", "2", CVAR_ARCHIVE);
	cvar_r_dlss_taa_history_weight_ultra_performance = Cvar_Get("r_dlss_taa_history_weight_ultra_performance", "0.85", CVAR_ARCHIVE);
	cvar_r_dlss_taa_history_weight_performance = Cvar_Get("r_dlss_taa_history_weight_performance", "0.88", CVAR_ARCHIVE);
	cvar_r_dlss_taa_history_weight_balanced = Cvar_Get("r_dlss_taa_history_weight_balanced", "0.91", CVAR_ARCHIVE);
	cvar_r_dlss_taa_anti_sparkle_scale = Cvar_Get("r_dlss_taa_anti_sparkle_scale", "0.5", CVAR_ARCHIVE);
	cvar_r_dlss_taa_variance_scale = Cvar_Get("r_dlss_taa_variance_scale", "0.85", CVAR_ARCHIVE);
...
#undef UBO_CVAR_LIST
	...
	/* Initialize Streamline BEFORE creating the Vulkan device so SL can
	   intercept vkCreateDevice (needed for future DLSS-G swapchain support). */
	vkpt_dlss_pre_init();
	...
	/* Provide Vulkan device info to Streamline for DLSS SR. */
	vkpt_dlss_init();
	...
	/* Apply initial Reflex mode from cvar (must come after slSetVulkanInfo). */
...
VkCommandBuffer vkpt_begin_command_buffer(cmd_buf_group_t* group)
{
	...
	_VK(vkpt_dlss_vkBeginCommandBuffer(cmd_buf, &begin_info));
	...
	VkFence fence)
{
	...
	VkSemaphore semaphores_local[8];
	VkPipelineStageFlags stages_local[8];
	uint32_t device_indices_local[8];
	uint64_t wait_values_local[8];
	int total_waits = 0;

	for (int i = 0; i < wait_semaphore_count && i < 8; ++i)
	{
		semaphores_local[total_waits] = wait_semaphores[i];
		stages_local[total_waits] = wait_stages[i];
		if (wait_device_indices)
			device_indices_local[total_waits] = wait_device_indices[i];
		wait_values_local[total_waits] = 0;
		total_waits++;
	}

	if (timeline_wait_semaphore != VK_NULL_HANDLE && total_waits < 8)
	{
		semaphores_local[total_waits] = timeline_wait_semaphore;
		stages_local[total_waits] = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
		if (wait_device_indices)
			device_indices_local[total_waits] = 0;
		wait_values_local[total_waits] = timeline_wait_value;
		total_waits++;
	}

	VkTimelineSemaphoreSubmitInfo timeline_submit_info = {
		.sType = VK_STRUCTURE_TYPE_TIMELINE_SEMAPHORE_SUBMIT_INFO,
		.waitSemaphoreValueCount = total_waits,
		.pWaitSemaphoreValues = wait_values_local,
	...
	VkSubmitInfo submit_info = {
		.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO,
		.pNext = (timeline_wait_semaphore != VK_NULL_HANDLE) ? &timeline_submit_info : NULL,
		.waitSemaphoreCount = total_waits,
		.pWaitSemaphores = total_waits ? semaphores_local : NULL,
		.pWaitDstStageMask = total_waits ? stages_local : NULL,
		.signalSemaphoreCount = signal_semaphore_count,
		.pSignalSemaphores = signal_semaphores,
		.commandBufferCount = 1,
		.pCommandBuffers = &cmd_buf,
	...
	VkDeviceGroupSubmitInfo device_group_submit_info = {
		...
		.waitSemaphoreCount = total_waits,
		.pWaitSemaphoreDeviceIndices = total_waits ? device_indices_local : NULL,
		.commandBufferCount = 1,
		.pCommandBufferDeviceMasks = &execute_device_mask,
		.signalSemaphoreCount = signal_semaphore_count,
		.pSignalSemaphoreDeviceIndices = signal_device_indices,
